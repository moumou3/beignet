...
enum DEBUGP_LEVEL
{
  ...
  do {                                                  \
    printk(KERN_DEBUG "Beignet: "#fmt, ##__VA_ARGS__);  \
    printk(KERN_DEBUG "\n");                              \
  } while (0)
#endif
...
/* Throw errors */
  ...
  #define ERR(ERROR, ...)                                             \
  do {                                                                \
    printk(KERN_DEBUG "error in %s line %i\n", __FILE__, __LINE__);     \
...
#define KZALLOC_STRUCT(T) (struct T*) cl_kzalloc(sizeof(struct T))
#define KZALLOC(T) (T*) cl_kzalloc(sizeof(T))
#define KZALLOC_ARRAY(T, N) (T*) cl_kzalloc(N * sizeof(T))
#define MEMZERO(x) do { memset((x),0,sizeof(*(x))); } while (0)
...
do {                                                    \
  ...
  printk(KERN_EMERG "### ASSERTION FAILED %s: %s: %d: %s\n",      \
               __FILE__, __func__, __LINE__, #x); dump_stack(); BUG_ON();  \
} while(0)
#endif
